...
class V_NewsletterSubscribersController extends VoyagerBaseController
{
    ...
    public function exportSubscribers(Request $request)
    {
        ...
        $subscribers = DB::table('newsletter_subscribers as NS')
                            ->leftJoin('users as U','U.id','NS.user_id')
                            ->select(
                                'NS.id as ns_id',
                                'NS.user_id as ns_user_id',
                                'NS.email as ns_email',
                                'NS.status as ns_status',
                                'U.name as u_name',
                                'U.last_name as ns_last_name'
                            )
                            ->get();

        $fileName = 'Export_NewsletterSubscribers_'.date_format(Carbon::now(),"d.m.Y H_i_s");

        $EXP = $subscribers;

        return (new FastExcel($EXP))->download($fileName.'.xlsx', function ($subscribers) {
            return [
                'ID' => $subscribers->ns_id,
                'E-mail' => $subscribers->ns_email,
                'Status' => $subscribers->ns_status,
                'Name' => $subscribers->u_name,
                'Last name' => $subscribers->ns_last_name,
            ];
        });
