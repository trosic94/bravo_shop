...
class V_SlidersItemsController extends VoyagerBaseController
{
    ...
    public function deleteSlide(Request $request)
    {
        ...
        $findeSLide = SlidersItems::where('id',request('slide_id'))->first();

        $sliderID = $findeSLide->slider_id;
        $slideTITLE = $findeSLide->title;

        if ($findeSLide):

            $findeSLide = SlidersItems::where('id',request('slide_id'))->delete();            

        endif;

        return redirect('/SDFSDf345345--DFgghjtyut-6/sliders/'.$sliderID.'/edit')
        ->with([
            'message'    => __('voyager::generic.successfully_deleted').": {$slideTITLE}",
            'alert-type' => 'success',
        ]);
    }
    ...
    public function insertSlide(Request $request)
    {
        ...
        if (request('image') != null ) {
            ...
            $new_name = $slide['slider_id'].'-'.date_format(Carbon::now(),"dmYHis").'-'.$image->getClientOriginalName();
            ...
            $path = public_path('storage/slides/'.$new_name);
        ...
        $slide['url_target'] = request('url_target');
        ...
        $insertSlide = SlidersItems::insert($slide);

        return redirect('/SDFSDf345345--DFgghjtyut-6/sliders/'.$slide['slider_id'].'/edit')
        ->with([
            'message'    => __('voyager::generic.successfully_added_new').": {$slide['title']}",
            'alert-type' => 'success',
        ]);
    }
    ...
    public function editSlide(Request $request)
    {
        ...
        if ($request->ajax()):
            ...
            $findSLide = SlidersItems::where('id',$slideID)->first();
            ...
            if ($findSLide):
                ...
                $editForm .= '<div class="form-group col-md-12">
                                ...
                                <input type="text" name="title" class="form-control" value="'.$findSLide->title.'" required>
                            </div>

                            <div class="form-group col-md-12">
                                <label class="control-label" for="text">'.trans('shop_admin.title_content').'</label>
                                <input type="text" name="text" class="form-control" value="'.$findSLide->text.'">
                            </div>

                            <div class="form-group col-md-12">
                                <label class="control-label" for="url">'.trans('shop_admin.title_url').'</label>
                                <input type="text" name="url" class="form-control" value="'.$findSLide->url.'">
                            </div>';

                $editForm .= '<div class="form-group col-md-12">
                                <label class="control-label" for="url_target">'.trans('shop_admin.title_url_target').'</label>
                                <select id="targetSEL" class="form-control select2" name="url_target">
                                    <option value="_self">'.trans('shop_admin.title_url_target_self').'</option>
                                    <option value="_blank">'.trans('shop_admin.title_url_target_blank').'</option>
                                </select>
                            </div>';
                ...
                if ($findSLide->image != ''):
                    $editForm .= '<div class="form-group col-md-12">
                                    <img src="/storage/slides/'.$findSLide->image.'" alt="'.$findSLide->title.'" class="img100">
                                </div>';
                endif;

                // proveravam status
                $check = '';
                if ($findSLide->status == 1):
                    $check = 'checked';
                endif;

                $editForm .= '<div class="form-group col-md-12">
                                <label class="control-label" for="slide_order">'.trans('shop_admin.title_order').'</label>
                                <input type="text" name="slide_order" class="form-control" value="'.$findSLide->slide_order.'" required>
                            </div>

                            <div class="form-group col-md-12">
                                <label class="control-label" for="status">'.trans('shop_admin.title_status').'</label>
                                <br>
                                <input type="checkbox" name="status" class="toggleswitch" '.$check.'>
                            </div>';

                $editForm .= '<input type="hidden" name="slide_id" value="'.$findSLide->id.'">';
                $editForm .= '<input type="hidden" name="slider_id" value="'.$findSLide->slider_id.'">';
            ...
            $editSlide = SlidersItems::where('id',$slideID)->update($slide);

            return redirect('/SDFSDf345345--DFgghjtyut-6/sliders/'.$slide['slider_id'].'/edit')
            ->with([
                'message'    => __('voyager::generic.successfully_updated').": {$slide['title']}",
                'alert-type' => 'success',
            ]);
